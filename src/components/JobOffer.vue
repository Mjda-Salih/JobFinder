  ...
  <div class="row text-center custom__form">
    <div class="col-md-8 col-lg-6 form__inputs">
      ...
      <a-alert
        v-if="errors"
        type="error"
        message="Can you please fill all fields"
        banner
      />
      <form @submit.prevent="handleSubmit">
        <div v-if="step == 0" class="form__change">
          ...
          <div class="form-group">
            ...
            <a-select v-model:value="formState.location" style="width: 100%">
              <a-select-option
          ...
          <div class="form-group">
            ...
            >
              ...
              <a-select-option value="apprenticeship"
                >Apprenticeship</a-select-option
              >
            </a-select>
          </div>
        ...
        <div v-else-if="step == 1" class="form__change">
          ...
          <a-tag
            ...
            :key="skill"
            closable
            @close="deleteSkill(skill)"
            color="#108ee9"
            style="margin: 5px"
            >{{ skill }}</a-tag
          >
          <div class="button__direction">
            <a-button type="primary" class="left" @click="prev"
              >Previous</a-button
            >
            <a-button type="primary" class="right" @click="next">Next</a-button>
          </div>
        ...
        <div v-else-if="step == 2" class="form__change">
          <div class="recap_info">
            ...
            <a-tag v-for="skill in formState.skills" :key="skill">{{
              skill
            }}</a-tag>
      ...
      >
        <template #extra>
          <router-link to="/"
            ><a-button key="console" type="primary">Home</a-button></router-link
          >
        </template>
      </a-result>
    </div>
...
export default {
  ...
  methods: {
    handleSubmit() {
      this.$store.dispatch("addJob", this.formState).then(() => {
        ++this.step;
        this.success = true;
      });
    },
    next() {
      if (this.step == 0) {
        ...
        if (
          ...
          this.formState.skills.length == 0
        ) {
          this.errors = true;
        } else {
          ++this.step;
